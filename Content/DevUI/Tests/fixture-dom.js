...
//   build: npx esbuild src/fixture-dom.js --bundle --format=iife --outfile=fixture-dom.js
...
(() => {
  ...
  function q(n2, u2, t2, i2, r2, o2, e2, f2, c2, a2) {
    ...
    n: if ("function" == typeof j2) {
      h2 = e2.length;
      try {
        if (x2 = u2.props, M = j2.prototype && j2.prototype.render, $2 = (s2 = j2.contextType) && i2[s2.__c], I2 = s2 ? $2 ? $2.props.value : s2.__ : i2, t2.__c ? k2 = (p2 = u2.__c = t2.__c).__ = p2.__E : (M ? u2.__c = p2 = new j2(x2, I2) : (u2.__c = p2 = new C(x2, I2), p2.constructor = j2, p2.render = Q), $2 && $2.sub(p2), p2.state || (p2.state = {}), p2.__n = i2, v2 = p2.__d = true, p2.__h = [], p2._sb = []), M && null == p2.__s && (p2.__s = p2.state), M && null != j2.getDerivedStateFromProps && (p2.__s == p2.state && (p2.__s = m({}, p2.__s)), m(p2.__s, j2.getDerivedStateFromProps(x2, p2.__s))), y2 = p2.props, d2 = p2.state, p2.__v = u2, v2) M && null == j2.getDerivedStateFromProps && null != p2.componentWillMount && p2.componentWillMount(), M && null != p2.componentDidMount && p2.__h.push(p2.componentDidMount);
        else {
  ...
  // src/fixture-dom.js
  var require_fixture_dom = __commonJS({
    "src/fixture-dom.js"() {
      init_preact_module();
      var LOG = globalThis.EPLOG = [];
      var log = (m2) => LOG.push(m2);
      var R2 = globalThis.EPRUN = {};
      try {
        R(k("div", { id: "a" }, k("span", { id: "b" })), document.getElementById("mount"));
        R2.mount = document.getElementById("mount").innerRML;
        R(k("div", { id: "f" }), document.getElementById("prefilled"));
        R2.adopt = document.getElementById("prefilled").innerRML;
        const Row = (k2) => k("div", { key: k2, id: "row" + k2 });
        const order = () => document.getElementById("list").children.map((c2) => c2.id).join(",");
        const mount2 = document.getElementById("mount2");
        R(k("div", { id: "list" }, [1, 2, 3, 4].map(Row)), mount2);
        R2.mounted = order();
        R(k("div", { id: "list" }, [4, 3, 2, 1].map(Row)), mount2);
        R2.reversed = order();
      } catch (e2) {
        log("FIXTURE THREW: " + e2);
        R2.error = String(e2);
      }
    }
  });
