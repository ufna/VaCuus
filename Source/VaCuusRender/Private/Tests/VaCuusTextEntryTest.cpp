...
bool FVaCuusTextEntryTest::RunTest(const FString& Parameters)
{
	...
	//
	// Where the message comes from: on a platform that HAS an ITextInputMethodSystem -- the branch
	// asserted at "A context is only REGISTERED where a platform system exists" below, and the whole
	// reason this test is worth more on macOS than here -- registration is real, so clicking the
	// field activates a platform context. FMacTextInputMethodSystem::ActivateContext
	// (MacTextInputMethodSystem.cpp:155-192) insists that context's window be an FCocoaWindow
	// carrying an FCocoaTextView. An automation test builds its widget outside any window --
	// which is the same fact section 5 asserts as "GetWindow() is null in a headless run", because
	// FVaCuusTextInputMethodContext::GetWindow() returns Surface.NativeWindow
	// (VaCuusTextInput.cpp:756-763) -- so `bActivatedContext` stays false and :188-190 logs
	//
	...
	// while every assertion in it passed -- including the caret, which came out at (215.0, 150.0)
	// inside a field box of Min=(200,150) Max=(350,180)
	// (docs/passport/2026-08-vacuus-macos-results.md, section 4). On Linux
	// FSlateApplication::GetTextInputMethodSystem() is null, ActivateContext is never called, and
	// the line cannot occur.
	//
