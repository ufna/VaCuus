...
/** VaCuus-dqs.4: the two arms of the catalogue. Generated by Tools/gen_catalog_art.py. */
static const TCHAR* GCatalogLooseVfsPath = TEXT("catalog_loose.rml");
 ...
 */
/**
 * THE CATALOGUE, IN TWO SPELLINGS (VaCuus-dqs.4). The same 200 icons at the same size in the
 * same grid; the ONLY difference is where the pixels come from -- 200 separate files against
 * 200 sprite rectangles of one atlas. Both documents are generated, so neither can drift from
 * the other by hand-editing.
 *
 * NEITHER SHIPS ART IN THE REPOSITORY: Tools/gen_catalog_art.py writes both, and its output is
 * gitignored. Run it once before toggling either of these, or the documents are not there.
 *
...
static FAutoConsoleCommand GCatalogNodesCommand(
	TEXT("vacuus.CatalogNodes"),
...
static FAutoConsoleCommand GCatalogLooseCommand(
	...
	FConsoleCommandDelegate::CreateLambda([] { Toggle(GCatalogLooseVfsPath); }));

static FAutoConsoleCommand GCatalogAtlasCommand(
	TEXT("vacuus.CatalogAtlas"),
	TEXT("Toggle the same catalogue drawn as sprites of one atlas. Needs Tools/gen_catalog_art.py to have run."),
	FConsoleCommandDelegate::CreateLambda([] { Toggle(GCatalogAtlasVfsPath); }));
