...
dominate) and the two document-side routes are in the passport's re-baseline section;
choosing shrink-the-workload vs re-baseline-the-row is an owner decision — decided
2026-08-02, route B: the row split into typical-scale ≤0.50 ms (the M2 figures stand)
and reference worst-case ≤1.2 ms steady avg (measured 1.05, ~14% headroom); the
reasoning is recorded in the passport's "The re-baseline" section, per spec §2(g). (3) The RAM
...
| UI-thread Update + command record | ≤0.50 ms/frame (typical HUD scale) · ≤1.2 ms steady avg (reference worst case — re-baselined 2026-08-02, route B) | gate — **measured M2: 0.052 ms avg / 0.113 ms p99 (M1 HUD), 0.023 ms avg / 0.060 ms p99 (M2 demo) @1080p (2026-07-30)**, Update+Record summed, 13k frames each. Note this is the M1/M2 subset, not the full reference HUD (~10× the element count, animated), so the ~4× headroom is not the final margin. **M6 (2026-08-02): the full 1,732-node reference HUD measured 1.077 ms avg Dev / 1.050 ms avg steady cooked-Shipping — over the original single 0.50 budget, which was set at M2 scale. Owner route B split the row: 0.50 stands at typical scale, the reference worst case budgets 1.2 ms steady avg (~14% headroom as measured). History, attribution and reasoning: the passport's "The re-baseline" section** |
