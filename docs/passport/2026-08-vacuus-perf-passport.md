...
- **JS sub-row**: cap enforced at 16 MiB (`JS_SetMemoryLimit`; boot line `cap=16 MB … boot
  ...
  collection).
- **GPU, reported separately (OWNER DECISION whether it counts against the 32 MB)**: per-view
  ...
  statement; keeping it out must be stated on the row so nobody reads 32 MB as total footprint.
...
- The GPU-in-or-out owner decision on row 5 (recommendation recorded above).
