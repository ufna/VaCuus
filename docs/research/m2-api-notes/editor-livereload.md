...
- IN_Q_OVERFLOW IS DROPPED, NOT ESCALATED. ProcessAllINotifyChanges skips overflowed events outright (DirectoryWatchRequestLinux.cpp:496 -- CORRECTED 2026-07-30 from :542-543, see the note at the inotify-backend section above) and never synthesizes FCA_RescanRequired. After a bulk operation (git checkout, npm build emitting the whole UI tree) you can silently lose changes. Offer a manual 'Reload All' console command / editor button as the escape hatch; do not treat the watcher as lossless.
