...
3. **Runtime:** load the asset → the loader memory-maps the payload automatically where supported, else falls back to a resident buffer — **on 5.8, "supported" means Win64/Android/iOS yes, Linux/macOS NO** (headline finding below). The VFS is span-based either way; `StealFileMapping()` decouples the region's lifetime from the UObject.
...
`FGenericPlatformProperties::SupportsMemoryMappedFiles()` returns **false** (GenericPlatformProperties.h:258-261). **Windows overrides to true** (WindowsPlatformProperties.h:76-79) — **and so do Android (AndroidPlatformProperties.h:118-121) and iOS (IOSPlatformProperties.h:71-74)**. The false answer is the *minority* one: **Linux and Mac are the platforms that do not override it** — both property structs derive from the generic (LinuxPlatformProperties.h:20-21, MacPlatformProperties.h:19-20; no `MemoryMapped` token in either file). `FFileIoStore::OpenMapped` hard-fails on that same property (IoDispatcherFileBackend.cpp:2124-2127), and the cook-side feature check resolves from the same properties (TargetPlatformBase.h:493-494) — so Linux/macOS cooks won't even set the flag. The OS capability exists (full `FUnixPlatformFile::OpenMappedEx2`/`MapRegion`, UnixPlatformFile.cpp:1331, :901) — it is the engine's *property gate* that says no.

**Consequence, scoped correctly (this paragraph previously said "literally true only on Win64", which was wrong the day it was written):** arch §9's "VFS reads memory-mapped bundle entries" is true on **every platform whose properties answer true — Win64 today, Android and iOS whenever the plugin is built there** — and false on Linux/macOS, which load the identical asset inline/resident. Win64 is only the platform where we have *observed* it. The honest product statement for the docs: *"memory-mapped where the engine's platform properties allow it — Win64, Android, iOS; a single resident buffer on Linux and macOS — and the VFS reads a span either way."* Optionally file the one-line `SupportsMemoryMappedFiles() => true` Linux/Mac property override as an engine suggestion — but v1 must not depend on engine patches.

**Mapping true does not mean mapping engages.** The property is a permission, not an outcome — on Android the mapped branch is expected to *fail* anyway, because `FAndroidPlatformFile::OpenMappedEx2` `open()`s `LocalPath` only and never consults the OBB/APK `AssetPath` (AndroidPlatformFile.cpp:1719-1746), so a staged Android build should land on the resident branch for a reason that has nothing to do with the property. That is exactly why the mount log line reports which of the two it observed rather than naming a platform (`VaCuusBundleMount.cpp`, the `FinishAndPublishLocked` log).
