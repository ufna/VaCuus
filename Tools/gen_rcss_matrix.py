...
def main():
    ...
    w.append("box's own 0..1 (VaCuusMaterial.usf). A material that declares customized UVs is")
    w.append("refused at registration with an Error naming the count: the replay pass")
    w.append("evaluates the pixel stage only, and Slate runs customized UVs in its vertex")
    w.append("shader.")
    w.append("")
